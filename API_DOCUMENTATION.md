...
## 🔐 قواعد الوصول في تطبيق الجوال للـ CRM والمشاريع

قوائم وتفاصيل `Lead`, `Opportunity`, `Quotation`, و`Project` في تطبيق الجوال لا تعرض كل السجلات التي يستطيع المستخدم رؤيتها في ERPNext فقط، بل يتم تضييقها لتكون سجلات المستخدم المرتبطة به مباشرة.

القواعد المطبقة:

- أولاً يتم احترام صلاحيات Frappe القياسية باستخدام `frappe.get_list` و`doc.has_permission`.
- بعد ذلك يتم عرض السجل فقط إذا كان مرتبطاً بالمستخدم الحالي بإحدى الطرق التالية:
  - المستخدم هو `owner` للسجل.
  - المستخدم موجود في حقل المسؤول مثل `lead_owner` أو `opportunity_owner`.
  - السجل مرتبط بـ `Sales Person` الخاص بموظف المستخدم، مثل `sales_person` في `Opportunity` و`Quotation`.
  - السجل مسند للمستخدم في `Assigned To`.
  - في المشاريع، يكون المستخدم مرتبطاً بالمشروع كـ Project User أو عبر حقول المسؤول المتاحة في المشروع.

السلوك المتوقع لتطبيق Flutter:

- إذا لم يظهر Lead/Opportunity/Quotation/Project في القائمة، فهذا يعني أنه ليس ضمن نطاق مستخدم الجوال أو لا يملك صلاحية قراءته.
- إذا حاول التطبيق فتح تفاصيل سجل خارج نطاق المستخدم، سيرجع السيرفر خطأ صلاحية.
- لا يعتمد التطبيق على فلترة الواجهة فقط؛ الفلترة مطبقة من الـ backend.

الهدف من هذه القاعدة أن يرى كل مستخدم في تطبيق الجوال سجلاته هو فقط: العملاء المحتملين، الفرص، عروض الأسعار، والمشاريع المرتبطة به.
